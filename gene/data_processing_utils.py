...
def window_truncate(feature_vectors, seq_len, sliding_len=None):
    """ Generate time series samples by truncating fixed-length windows from a 2D array.
    Parameters
    ----------
    feature_vectors: ndarray of shape [total_length, feature_num]
    seq_len: int, window length
    sliding_len: int, sliding step size
    Returns
    -------
    ndarray of shape [num_windows, seq_len, feature_num]
    """
    sliding_len = seq_len if sliding_len is None else sliding_len
    total_len = feature_vectors.shape[0]

    # 最后一个 idx 要确保 idx + seq_len <= total_len
    start_indices = np.arange(0, total_len - seq_len + 1, sliding_len)

    sample_collector = []
    for idx in start_indices:
        sample_collector.append(feature_vectors[idx: idx + seq_len])

    return np.asarray(sample_collector, dtype=np.float32)
...
def add_artificial_mask(X, artificial_missing_rate, set_name):
    ...
    if set_name == "train":
        ...
        # 强制转换为整数索引或布尔掩码
        if indices_for_holdout.dtype == bool:
            pass  # 布尔掩码无需处理
        else:
            indices_for_holdout = indices_for_holdout.astype(int)
