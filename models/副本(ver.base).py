...
def state_carry_forward(X, masks):
    """
    X: (B,T,D)
    masks: (B,T,D)  1=observed, 0=missing
    返回 lov: (B,T,D)，表示每个时刻前一个观测值；
          若首位有观测，则 lov[0] = X[0]。
    """
    B, T, D = X.shape
    device = X.device

    idx_arange = torch.arange(T, device=device).view(1, T, 1)
    pos = masks.long() * (idx_arange + 1)
    cummax_vals, _ = pos.cummax(dim=1)

    last_idx_prev = torch.zeros_like(cummax_vals)
    if T > 1:
        last_idx_prev[:, 1:, :] = cummax_vals[:, :-1, :]

    idx0 = (last_idx_prev - 1).clamp(min=0).long()
    X_perm = X.permute(0, 2, 1)
    idx0_perm = idx0.permute(0, 2, 1)
    gathered = torch.gather(X_perm, dim=2, index=idx0_perm)
    lov = gathered.permute(0, 2, 1)
    ...
    # === 新增边界条件：仅修改首位 ===
    first_obs = masks[:, 0, :] == 1
    lov[:, 0, :][first_obs] = X[:, 0, :][first_obs]

    return lov


def state_carry_backward(X, masks):
    """
    ...
    lov_rev = state_carry_forward(X_rev, masks_rev)
...
class IFSAmodel(nn.Module):
    ...
    def impute(self, inputs):
        ...
        # deal with missing
        X_stem = linear_stem(X, masks)

        # FFT
        fft_vals = torch.fft.fft(X_stem, dim=1)
        fft_vals_filtered = fft_vals.clone()

        T = X_stem.shape[1]
        k = min(self.low_pass_n, T // 2)

        # low-pass filtering
        fft_vals_filtered[:, k:-k, :] = 0
        lov = state_carry_forward(X, masks)  # (B,T,D)
        nov = state_carry_backward(X, masks)  # (B,T,D)

        # IFFT
        ts_reconstructed = torch.fft.ifft(fft_vals_filtered, dim=1).real  # (B,T,D)

        # fill missing in X with fft-reconstructed
        X_filled = torch.where(masks.bool(), X, ts_reconstructed)

        # embeddings
        m_emb = self.mask_embedding(masks)  # B,T,D,emb
        B, T, D = X.shape
        X_filled_flat = X_filled.view(B, T, D)
        m_emb_flat = m_emb.view(B, T, D * self.mask_emb_dim)
        lov_flat = lov.view(B, T, D)
        nov_flat = nov.view(B, T, D)
        fused = torch.cat([X_filled_flat, lov_flat, nov_flat, m_emb_flat], dim=-1)  # (B,T,fused_dim)
        ...
        for encoder_layer in self.layer_stack:
            enc_output, atten = encoder_layer(enc_output, time_points)
