...
class Controller:
    def __init__(self, early_stop_patience, disable_early_stop_before_n_epochs):
        self.original_early_stop_patience_value = early_stop_patience
        self.early_stop_patience = early_stop_patience
        self.disable_early_stop_before_n_epochs = disable_early_stop_before_n_epochs
    ...
    def __call__(self, stage, info=None, logger=None):
        if stage == "train":
            ...
            if info is not None:
                # 尝试获取 imputation_MAE
                train_loss = info.get("imputation_MAE", None)

                # 如果 loss 是 None 或 0，就直接不记录/不打印
                if train_loss is None or (
                        isinstance(train_loss, torch.Tensor) and train_loss.item() == 0) or train_loss == 0:
                    return self.state_dict

                # 记录训练损失
                if isinstance(train_loss, torch.Tensor):
                    train_loss = train_loss.detach().cpu().item()
                self.current_epoch_train_losses.append(float(train_loss))
            ...
            # 打印当前 epoch 的平均训练损失（仅当存在有效记录时）
            if self.current_epoch_train_losses and logger:
                avg_train_loss = sum(self.current_epoch_train_losses) / len(self.current_epoch_train_losses)
                # logger.info(f"average training loss before validation: {avg_train_loss:.6f}")

            # 更新最优验证 MAE
            imputation_MAE_dropped = False
            if current_imputation_MAE < self.state_dict["best_imputation_MAE"]:
                logger.info(f"[Best] validating imputation MAE updated: {current_imputation_MAE:.6f}")
                self.state_dict["best_imputation_MAE"] = current_imputation_MAE
                imputation_MAE_dropped = True
            else:
                logger.info(f"validating imputation MAE: {current_imputation_MAE:.6f}")

            if imputation_MAE_dropped:
                self.state_dict["save_model"] = True

            # --- Early stopping logic ---
            if self.state_dict["epoch"] >= self.disable_early_stop_before_n_epochs:
                # 仅在达到禁用期之后才启用 early stopping
                if self.state_dict["save_model"]:
                    self.early_stop_patience = self.original_early_stop_patience_value
                else:
                    if self.early_stop_patience > 0:
                        self.early_stop_patience -= 1
                    elif self.early_stop_patience == 0 and logger:
                        logger.info("early_stop_patience exhausted, stop training")
