...
class LoadDataset(Dataset):
    def __init__(self, file_path, seq_len, feature_num, model_name):
...
class LoadValTestDataset(LoadDataset):
    ...
    def __init__(self, file_path, set_name, seq_len, feature_num, model_name):
        super(LoadValTestDataset, self).__init__(
            file_path, seq_len, feature_num, model_name
        )
    ...
    def __getitem__(self, idx):
        if self.model_name in ["Transformer", "SAITS", "CECFmodel", "ImputeFormer", "TimesNet"]:
            sample = (
                torch.tensor(idx),
            ...
            assert ValueError, f"Error model type: {self.model_name}"
        return sample
...
class LoadTrainDataset(LoadDataset):
    ...
    def __init__(
        self, file_path, seq_len, feature_num, model_name, masked_imputation_task
    ):
    ...
    def __getitem__(self, idx):
        X = self.X[idx]
        # X_median = self.X_median[idx]
        if self.masked_imputation_task:
            X = X.reshape(-1)
            valid_indices = np.where(~np.isnan(X))[0]
            sample_num = int(round(len(valid_indices) * self.artificial_missing_rate))
            indices = np.random.choice(valid_indices, sample_num, replace=False)
            ...
            if X.size == self.seq_len * self.feature_num:
                X = X.reshape(self.seq_len, self.feature_num)
            else:
                raise ValueError(f"Cannot reshape data of size {X.size} to ({self.seq_len},{self.feature_num})")
            ...
            if self.model_name in ["Transformer", "CECFmodel", "SAITS", "ImputeFormer", "TimesNet"]:
                sample = (
                    torch.tensor(idx),
...
class UnifiedDataLoader:
    def __init__(
        ...
        model_name: model type, determine returned values;
    ...
    def get_train_val_dataloader(self):
        ...
        self.val_dataset = LoadValTestDataset(
            self.dataset_path, "val", self.seq_len, self.feature_num, self.model_name
        )
    ...
    def get_test_dataloader(self):
        self.test_dataset = LoadValTestDataset(
            self.dataset_path, "test", self.seq_len, self.feature_num, self.model_name
        )
    ...
    def prepare_dataloader_for_imputation(self, set_name):
        data_for_imputation = LoadDataForImputation(
            self.dataset_path, set_name, self.seq_len, self.feature_num, self.model_name
        )
